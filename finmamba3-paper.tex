...
kernel---a surviving modulation would have to occupy.
...
world-model architecture is \emph{not necessary} for the economic edge.  The
logistic/GBT/naive rows are on the recorded vintage (re-fitting on $1.6$M
training ticks exceeds our compute budget); the settlement and EV rows are
current.  The conclusion is vintage-robust: the recorded logistic ($+\$7{,}016$)
exceeds even the \emph{higher} current settlement total ($+\$5{,}147$).}
\label{tab:baseline}
\end{table}
...
PnL of the same-feature simple models on the $645$-market set (recorded
vintage), with their \emph{market-block} $95\%$ confidence intervals (resampling
whole markets).  The logistic's point total ($+\$7{,}016$) exceeds both
world-model heads (dashed/dotted), and its market-block interval ($+\$2{,}928$
to $+\$11$k over $243$ independent markets) survives the dependence-respecting
bootstrap; the deflationary conclusion holds across vintages, the logistic
exceeding even the higher current settlement total ($+\$5{,}147$).}
\label{fig:baseline}
\end{figure}
...
\emph{both} at market-block P(profit)$=0.994$ (recorded vintage)---robustly
positive in exactly the low-predictability regime where the world model's
settlement head is \emph{weak} ($+\$51$ at P(profit)$=0.63$ on the current
vintage; Table~\ref{tab:ev_arms}).  A linear readout of the spot features
