...
microstructure adapting the Drama agent~\citep{wang2024drama}, and use it
to ask whether conditioning a structured state-space backbone on an
\emph{inferred} market regime improves behaviour under distribution
shift.  Our mechanism, \emph{RegimeFiLM}, applies a zero-initialized,
per-block channel-wise affine transform to the Mamba input, propagating
an inferred regime into the input-dependent selective-scan parameters
$\Delta,\mathbf{B},\mathbf{C}$ without modifying the CUDA kernel.  Across
four regime-dependent forecasting objectives on FI-2010 and Polymarket
and a simulated-trading (PnL) objective, RegimeFiLM is a \textbf{decisive
null}: it collapses to identity under joint training and cannot be forced
active even by direct supervision.  The null is dataset-, asset-, and
resolution-independent---replicating on the Kaggle crypto spot book (BTC,
ETH, ADA; 1\,s/1\,min/5\,min)---and we explain it mechanistically: an
input-side affine is a \emph{gauge} direction that folds losslessly into
the block's own projections, so joint training returns it to identity.
The same effort yields a positive economic result the architecture did
not.  Given the underlying spot path that \emph{defines} settlement, the
model's over-confident-but-directional settlement probability---traded by
a selective-participation strategy that bets only in forecastable
regimes---earns \textbf{+\$4{,}729} on 408 held-out Polymarket BTC markets
(a byte-reproducible deterministic-latent anchor) and beats a fair
trend-following baseline by \textbf{6--9$\times$} across two
stochastic-latent seeds, survivable under a fat-tailed bootstrap.
Crucially, this edge is \emph{architecture-independent}: a logistic
regression on the same features reproduces and exceeds it ($+\$7{,}016$),
so the spot-conditioned pipeline and selective-participation
framing---not the sequence model---carry the economic result.  Broader
validation (645 markets) exposes a latent low-predictability failure in
the settlement head (P(profit)$=0.276$); supervising the \emph{per-tick
expected value} directly---our \textbf{BookRelativeEdgeHead}---lifts total
held-out PnL $+41\%$ and collapses the high-vs-low regime spread
