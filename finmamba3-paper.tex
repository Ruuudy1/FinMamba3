...
across datasets (Section~\ref{sec:results_ablation}).  We therefore
report the backbone ablation as a finding rather than a selling point,
and none of the results below depend on the choice of sequence model.
...
markets) keeps the edge alive exactly where the gate routes it: across the
high-predictability markets it holds $P(\text{profit})=1.0$ with a $95\%$ CI of
$[+\$2{,}686,\,+\$7{,}951]$ over $129$ independent traded markets, while the
low-predictability markets straddle zero ($P(\text{profit})=0.53$, CI
$[-\$3{,}669,\,+\$3{,}681]$ over $139$)---the market-level form of the same
low-predictability failure the BookRelativeEdgeHead removes
(Section~\ref{sec:results_ev}).
...
hardware envelope and we make no claim about it.
