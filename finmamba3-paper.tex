...
% ==============================================================================
\title{FinMamba3: Regime Conditioning Fails, Selective Participation Pays\\A Mamba-3 World Model for Prediction-Market Microstructure}
...
clock, and an honest per-tick target, its calibrated settlement
probability---traded by a selective-participation strategy that bets only
in forecastable regimes---earns \textbf{+\$4{,}729} on held-out
Polymarket BTC (deterministic), beats a fair trend-following baseline by
\textbf{6--9$\times$}, and is fully survivable under a fat-tailed
bootstrap (P(profit)$=1.0$, worst-5\% drawdown $4.7\%$).  We characterize
...
oracle-repricing lag.  The regime axis a FiLM router will not encode is
exactly the one a strategy gate harvests profitably.
\end{abstract}
...
can, and the resulting selective-participation strategy is the paper's
positive contribution.

We adapt the Drama world-model framework~\citep{wang2024drama} to LOB
microstructure.  Our system, \textbf{FinMamba3}, makes three
contributions.

\paragraph{(1) Mamba-3 MIMO backbone for LOB sequences.}
We replace Drama's Mamba-2 sequence model with a Mamba-3 MIMO
backbone~\citep{lahoti2026mamba3}, a structured state-space model with
linear-time complexity and multi-input multi-output state transitions.
We deploy it as the dynamics backbone of a financial world model and
include it as one arm of a controlled architecture ablation rather than
asserting an advantage in advance.
\todo{Verify the Mamba-3 reference (\texttt{lahoti2026mamba3}) against a
citable arXiv/venue record before submission; remove any performance
figures that cannot be sourced from the published paper.}
...
proxy.  Conditioned on the spot path, the settlement head becomes
calibrated enough that a selective-participation strategy---betting only
when a causal predictability gate fires---turns its probabilities into a
survivable, naive-beating economic edge (Section~\ref{sec:results}).
This is the paper's positive result, and it is
\emph{architecture-independent}: it does not depend on RegimeFiLM.
...
inherits the DreamerV3/Drama architecture but upgrades the backbone to
Mamba-3 MIMO and adds explicit regime conditioning, a capability we have
not found in prior world models.
...
\todo{Replace or verify the quantitative half-spread figure and its
source (\texttt{sotskov2025polymarket}); the cited arXiv identifier
could not be confirmed and the specific basis-point figure should not be
stated until a citable measurement is available.}
...
$q_\phi(\zt \mid e_t, \ht) = \mathrm{Cat}(\zt; \bm{\pi}^{\text{post}}_t)$
...
\begin{equation}\label{eq:sequence_model}
  \ht = f_\theta(\zt, \at, h_{t-1}),
\end{equation}
from which a prior $p_\theta(\zt \mid \ht) = \mathrm{Cat}(\zt;
...
$\mathcal{L}_{\text{dyn}} = \KL[q_\phi(\zt \mid e_t, \ht)\,\|\,
\mathrm{sg}[p_\theta(\zt \mid \ht)]]$ trains the prior to match the
...
\todo{State any quantitative claims about Mamba-3 (e.g.\ relative
perplexity or downstream gains) only with a verifiable citation to the
source paper; otherwise describe MIMO as a design choice evaluated in our
...
(Section~\ref{sec:aux_heads}).  \todo{Add Figure~1: architecture
overview (encoder $\to$ posterior $\to$ Mamba-3 backbone with RegimeFiLM
$\to$ heads).}
...
and accommodates different numbers of levels.  We evaluate this encoder
against a flat-MLP encoder in the ablation of
Section~\ref{sec:experiments} rather than assuming it is superior.
...
final hidden state is $\ht = \RMSNorm(x_t^{(L)}) \in \R^{d_h}$.
...
inference, and which write policy is preferable, is left to the ablation
...
$\lambda_{\text{H}} = 0.1$, $\lambda_{\text{S}} = 0.25$, and
$\lambda_{\text{ent}} = 0.01$.  Reward and termination heads are gated
...
by betting only where the underlying is forecastable.  As an anti-artifact
...
\paragraph{Secondary experiments.}
(a)~\emph{Backbone ablation}: Mamba-1/2/3-SISO/3-MIMO vs.\ Transformer on
reconstruction and direction accuracy; failure criterion: Mamba-3 MIMO
does not match or exceed the best alternative under matched parameters.
(b)~\emph{Episodic memory}: FIFO vs.\ novelty-filtered vs.\ off; failure
criterion: neither memory policy improves out-of-regime metrics over the
parametric model.
(c)~\emph{Auxiliary heads}: leave-one-out over the direction, Hawkes, and
settlement heads; failure criterion: removing a head leaves downstream
metrics unchanged within noise, indicating it provides no useful
inductive bias.
...
The evaluation produced a \textbf{dual outcome}: the regime-conditioning
architecture is a decisive negative, and the spot-conditioned pipeline
plus a selective-participation strategy---built to test it---is a
genuine, survivable economic positive.
...
its calibrated settlement probability, traded by the selective-
participation strategy, is profitable on held-out Polymarket BTC and
clears every anti-artifact gate (Table~\ref{tab:edge}).  The edge is
robust across two model seeds at a \emph{single} ER gate fit once on
train and frozen, and a deterministic latent makes it byte-reproducible
at $+\$4{,}729$ (1{,}553 trades, P(profit)$=1.000$, drawdown $0.045$).
The predictability gate alone harvests only $+\$513$; the model's
calibrated probability is the dominant driver ($+\$4{,}048$ over
trend-following).
...
\bottomrule
\end{tabular}
\caption{Held-out BTC, train-frozen ER gate $=0.60$, versus a fair
predictability-gated trend-following naive.  Both seeds clear every gate
(P(profit)$>0.7$, drawdown $<0.15$, beats naive).}
\label{tab:edge}
\end{table}
...
\bottomrule
\end{tabular}
\caption{Deterministic BTC.  The conjunction lifts the per-trade edge
$+63\%$ and cuts drawdown $-36\%$ versus the predictability-only
headline, all at P(profit)$=1.000$.}
...
needs only the right side, and the model has it ($0.785 \gg 0.5$).  It is
...
takeable, so live market impact would erode some of the edge; and the
...
depth ceiling, by contrast, is train-justified).  The backbone ablation
...
hardware.
\todo{Insert the backbone-ablation and episodic-memory tables once the
matched-parameter runs (including the A100 MIMO cell) are complete; the
present results stand on the regime-conditioning null and the economic
edge, which do not depend on them.}
...
distribution shift.  The result is a \textbf{dual outcome}.
...
\emph{The economic question is a genuine positive the architecture did
not provide.}  Rebuilding the pipeline to expose the spot path that
defines settlement, and judging the model by simulated PnL, yields a
calibrated settlement probability whose divergence from the
oracle-lagged book---traded by a selective-participation strategy---earns
$+\$4{,}729$ on held-out BTC (deterministic), beats a fair
trend-following baseline by $6$--$9\times$, and is fully survivable.  The
edge is a three-way microstructure conjunction (predictable,
liquid-but-not-efficient, early-in-lifecycle) driven by an
over-confident-but-directional head; it is architecture-independent.  The
regime axis a FiLM router will not encode is exactly the one a strategy
gate harvests profitably.

\paragraph{Future work.}
The negative motivates a cross-dataset confirmation (the regime A/B and
the backbone ablation on a public crypto spot-LOB corpus) and the
A100-class MIMO headline run the 4080 cannot host.  The positive
motivates a live-execution study (the present edge is a backtest with
walk-the-book fills) and an explicitly calibrated settlement head, since
calibration is what currently bars convexity-based (Kelly) sizing.
...
\newblock 2026.
\newblock \todo{Confirm authors, venue, and arXiv identifier; this entry
could not be verified against a public record at time of writing.}
...
\bibitem[Sotskov et~al.(2025)]{sotskov2025polymarket}
D.~Sotskov et~al.
\newblock Market microstructure of decentralized prediction markets.
\newblock 2025.
\newblock \todo{Citation unverified; confirm authors, venue, and a valid
arXiv identifier (the previously listed ID could not be confirmed) before
relying on any quantitative figure from this source.}
