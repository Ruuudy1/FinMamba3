...
% ==============================================================================
\title{FinMamba3: Regime Conditioning Fails, Selective Participation Pays\\A World Model for Prediction-Market Microstructure}
...
% ==============================================================================
\begin{abstract}
Financial markets are non-stationary, partially observable environments
whose latent dynamics shift across regimes.  We present
\textbf{FinMamba3}, a Mamba-3 world model for limit order book (LOB)
microstructure that adapts the Drama agent~\citep{wang2024drama}, and
use it to ask whether conditioning a structured state-space backbone on
an \emph{inferred} market regime improves behaviour under distribution
shift.  Our central mechanism, \emph{RegimeFiLM}, applies per-block,
channel-wise affine transforms to the Mamba input; because the
selective-scan parameters $\Delta$,~$\mathbf{B}$, and~$\mathbf{C}$ are
input-dependent, this propagates an inferred regime into the
state-space dynamics through the selection mechanism itself, without
modifying the CUDA kernel, and is zero-initialized so the modulated
model is \emph{exactly} the unmodulated baseline at initialization.
Across four regime-dependent forecasting objectives (volatility-scale,
order-flow volume, direction macro-F1, binary settlement) on FI-2010 and
Polymarket, and a simulated-trading (PnL) objective, RegimeFiLM is a
\textbf{decisive null}: it collapses to identity under joint
training---the $\gamma$ deviation decays toward zero and the router
entropy sits at its uniform maximum---and cannot be forced active even
by direct predictability supervision.  The null is dataset-, asset-, and
resolution-independent---it replicates on the Kaggle high-frequency crypto
spot book (BTC, ETH, ADA; 1\,s/1\,min/5\,min) under a predictability regime
axis, where even a forced-active, directly-supervised modulation provably
decays to identity.  The same effort yields a positive
economic result the architecture did not.  Once the model is given the
underlying spot path that \emph{defines} settlement, a time-to-expiry
clock, and an honest per-tick target, its over-confident-but-directional
settlement probability---traded by a selective-participation strategy
that bets only in forecastable regimes---earns \textbf{+\$4{,}729} on 408
held-out Polymarket BTC markets (deterministic), earning \textbf{6--9$\times$}
the PnL of a fair trend-following baseline across two model seeds, and is
survivable under a fat-tailed bootstrap (per-trade P(profit)$=1.0$,
worst-5\% drawdown $4.5\%$; the market-block bootstrap that respects
intra-market dependence is reported in Section~\ref{sec:results_edge}).
Crucially, this edge is \emph{architecture-independent}: a logistic
regression on the same spot-conditioned features reproduces it
($+\$7{,}016$, exceeding the world model's own settlement head), so the
pipeline and selective-participation framing---not the sequence model---carry
the economic result.  We characterize
the edge as a three-way microstructure conjunction---predictable,
liquid-but-not-efficient, and early-in-lifecycle---and trace it to an
over-confident-but-directional settlement head exploiting the
oracle-repricing lag.  Broader validation (645 markets) then exposes a
latent regime failure in the settlement head (P(profit)$=0.276$ on
low-predictability markets); supervising the \emph{per-tick expected
value} directly---our \textbf{BookRelativeEdgeHead}---converts the
$-\$826$ low-predictability loss into $+\$2{,}548$, lifts total PnL to
$+\$5{,}565$ ($+41\%$), and collapses the high-vs-low PnL spread
$12{\times}$.  The regime axis a FiLM router will not encode is exactly
the one a strategy gate and a direct-EV head, together, harvest
profitably.
\end{abstract}
...
modeling---and stress-testing it as the dynamics backbone of a world model
under genuine non-stationarity, where the central question is not perplexity
but whether an \emph{inferred} latent regime can be made to help.  Our
system, \textbf{FinMamba3}, makes five contributions.

\paragraph{(1) A selective state-space world model for LOB microstructure.}
We cast LOB microstructure as a POMDP world model in the DreamerV3/Drama
lineage~\citep{hafner2023dreamerv3, wang2024drama} and instantiate its
dynamics with a Mamba-3 selective state-space
backbone~\citep{lahoti2026mamba3}.  The backbone is an implementation
choice, not a claimed advantage: under matched parameters \emph{no}
backbone (Mamba-1, Mamba-2, Mamba-3 SISO, or a Transformer) dominates
across datasets, and the multi-input multi-output (MIMO) configuration
that might break the tie is structurally infeasible on our hardware---it
exceeds the RTX~4080 shared-memory cap
(Section~\ref{sec:results_ablation}).  We therefore report the backbone
ablation as a finding rather than a selling point, and none of the
results below depend on the choice of sequence model.

\paragraph{(2) Regime-aware FiLM modulation of the selective scan.}
We introduce a \emph{RegimeFiLM modulator} that infers a categorical
latent regime from the backbone's stem representation and emits
per-block, channel-wise affine parameters (scale~$\gamma$ and
shift~$\beta$).  Because the selective-scan parameters $\Delta$,
$\mathbf{B}$, and $\mathbf{C}$ in Mamba are input-dependent
(Section~\ref{sec:ssm_prelim}), applying a FiLM transform to each
block's input propagates the regime signal \emph{into} the state-space
dynamics without modifying the CUDA kernel.  A load-balancing
regularizer discourages regime collapse.  We are not aware of prior work
that conditions selective-scan dynamics on an inferred regime in a
financial world model.  Our central empirical finding is that, despite
an exact zero-initialized control and the strongest activation pressure
we could apply, this mechanism does \emph{not} improve robustness under
distribution shift on any objective we test: it is a clean,
mechanistically-characterized null (Section~\ref{sec:results}).

\paragraph{(3) Microstructure-aware encoder, features, and auxiliary
heads.}
We replace the convolutional image encoder with a Transformer that
treats each depth level of the LOB as a token, enriched with
binary-market invariants (boundary distance, logit-space velocity,
Amihud illiquidity, variance ratio).  We further implement Lopez de
Prado bar aggregation for tick-stream denoising and three auxiliary
heads---directional prediction, Hawkes event-intensity, and binary
settlement---that shape the latent space toward financially relevant
structure.

\paragraph{(4) Spot-conditioned pipeline and a profitable
selective-participation strategy.}
We rebuild the Polymarket pipeline to expose the underlying spot path
that \emph{defines} settlement, a time-to-expiry clock, an honest
per-tick (TTE-weighted) settlement target, and contemporaneous
cross-interval context, and we judge the model by \emph{simulated PnL}
through a decoupled order-matching backtester rather than a forecasting
proxy.  Conditioned on the spot path, the settlement head becomes directionally
informative---over-confident in magnitude but reliable in direction---so
that a selective-participation strategy betting only when a causal
predictability gate fires turns its probabilities into a survivable,
naive-beating economic edge (Section~\ref{sec:results}).
This is the paper's first positive result, and it is
architecture-independent: a logistic regression on the same features
reproduces and exceeds it (Section~\ref{sec:results_baseline}), so the
contribution is the spot-conditioned pipeline and the selective-participation
framing, not the sequence model that consumes them.

\paragraph{(5) Book-relative EV head for regime-robust trading.}
Broader validation exposes a latent failure of the settlement head:
P(profit)$=0.276$ on low-predictability markets.  We add a
\textbf{BookRelativeEdgeHead} that supervises the per-tick expected value
of buying each side of the contract net of the bid-ask spread, masking
ticks where the book is absent.  The training signal is explicitly zero
when the book already reflects the edge, teaching the model to abstain
in low-information regimes.  A single configuration (EV Huber loss only)
eliminates the settlement head's failure in low-predictability markets
and lifts total held-out PnL $+41\%$ while collapsing the high-vs-low
PnL gap $12{\times}$ (Section~\ref{sec:results_ev}).
...
\citet{nagy2025lobs5} propose LOBS5, an autoregressive token-level
...
FinMamba3 it is a stack of Mamba-3 MIMO blocks
...
dimension and an optional output-projection normalization.
Whether MIMO improves over SISO on LOB data is evaluated in our
own ablation (Section~\ref{sec:experiments}).
...
% --------------------------------------------------------------------------
\subsection{Mamba-3 MIMO Sequence Backbone}\label{sec:mamba3}
...
MIMO blocks.  Each block applies a pre-norm residual,
\begin{equation}\label{eq:mamba_block}
  x_t^{(\ell)} = x_t^{(\ell-1)} + \mathrm{Mamba3}\!\bigl(
  \RMSNorm(x_t^{(\ell-1)})\bigr),
  \qquad \ell = 1, \ldots, L.
\end{equation}
The Mamba-3 block parameterizes a selective state-space model with
$d_{\text{state}} = 128$, head dimension 64 (8 heads for $d_h = 512$),
MIMO rank~4, chunk size~16, and RoPE on 50\% of the head dimension.  The
final hidden state is $\hstate = \RMSNorm(x_t^{(L)}) \in \R^{d_h}$.

The MIMO parameterization couples information across heads within each
block via a low-rank cross-head mixing matrix.  We provide matched YAML
configurations for Mamba-1, Mamba-2, Mamba-3 SISO, Mamba-3 MIMO, and a
Transformer backbone so that the architecture ablation can be reproduced
with a single command per variant; whether MIMO improves over SISO on
LOB data is left to that ablation.
...
$P(\text{profit})=1.0$ should be read against its $\le\!408$ (resp.\ $645$)
independent markets; the harness emits the block statistic on any re-run.
...
Mamba-3 SISO, Mamba-3 MIMO, and a Transformer backbone under matched
...
LOB models LOBS5~\citep{nagy2025lobs5} and the diffusion approach
...
(Section~\ref{sec:results_ablation}); the pre-registered failure criterion
is that Mamba-3 MIMO does not match or exceed the best alternative, but the
MIMO arm is hardware-gated and unrun (Section~\ref{sec:results_ablation}), so
the realized comparison is among the non-MIMO backbones.  Three further
...
BookRelativeEdgeHead eliminates the settlement head's latent low-predictability
failure, lifting total PnL $+41\%$ and collapsing regime sensitivity
$12{\times}$.
...
\toprule
Family & Objective & Dataset & Gap (sign) & $\mathrm{film}_g$ & Verdict \\
\midrule
Volatility/scale & StudentT NLL    & FI-2010    & $\approx 0$ & $\approx$ ident.\ & \textsc{null} \\
Order-flow volume & Volume NLL     & FI-2010    & $\approx 0$ & $\approx$ ident.\ & \textsc{null} \\
Direction        & macro-F1        & FI-2010    & $+0.008$\,\dag & $0.018$ & \textsc{null} \\
Settlement       & BCE log-loss    & Polymarket & $+0.18$\,\dag  & $0.008$ & \textsc{null} \\
...
equals its uniform maximum ($\log R$) in all rows.  \dag~a non-zero gap
...
regardless of objective, regime axis, or forced supervision.
...
\caption{\textbf{Strongest-null test.} Forcing the modulation active
($\mathrm{film}_g = 0.250$ at step~0) and supervising the router directly on the
efficiency-ratio bucket still decays $\mathrm{film}_g$ monotonically toward identity on both
datasets (top), with the router entropy $\mathrm{reg}_H$ pinned at its uniform maximum
$\log 4$ throughout (bottom). An exponential fit gives $R^2 > 0.999$ with a dataset-invariant
time-constant $\tau \approx 7000$ and an asymptote $\le 0$, so the modulation provably
...
$W_\Delta/W_B/W_C$ directly, gating \emph{after} the scan, or conditioning a
quantity the block does not immediately re-project---are not trivially
absorbable and are the natural next test.  Our contribution is the negative
...
on the expanded 645-market set scores $+\$3{,}946$
...
The head is over-confident but directionally informative, which is what
the divergence trade exploits.}
\label{tab:calib}
\end{table}
...
depth ceiling, by contrast, is train-justified); and the settlement head
catastrophically fails in low-predictability markets (P(profit)$=0.276$,
drawdown $0.257$), a limitation addressed by direct EV supervision in
Section~\ref{sec:results_ev}.  The backbone ablation
(Mamba-1/2/3-SISO/MIMO vs.\ Transformer) and episodic-memory study are
reported on FI-2010 and Polymarket; the MIMO headline configuration
exceeds the RTX~4080 shared-memory cap and is deferred to A100-class
hardware.  The backbone ablation is presented in
Table~\ref{tab:backbone_ablation}; the episodic-memory study is deferred
to future work.
...
\midrule
Logistic regression (same features)      & $\mathbf{+7{,}016}$ & $\mathbf{0.146}$ \\
Gradient-boosted trees (same features)   & $+876$ & $0.149$ \\
...
world-model architecture is \emph{not necessary} for the economic edge.}
\label{tab:baseline}
\end{table}
...
PnL of the same-feature simple models on the $645$-market set, with their
\emph{market-block} $95\%$ confidence intervals (resampling whole markets).
The logistic regression's interval ($+\$2{,}928$ to $+\$11$k over $243$
independent markets) sits entirely above both world-model heads (dashed/dotted),
so its advantage survives the dependence-respecting bootstrap, not just the iid
one.}
\label{fig:baseline}
\end{figure}
...
(out-of-time) comparison: the $+\$7{,}016$ is an out-of-sample number on data
postdating every training tick, not an in-sample fit.

The result is deflationary by design and we report it as such: a plain
logistic regression on the same features earns $+\$7{,}016$ at Brier $0.146$,
exceeding both the settlement head ($+\$3{,}946$, Brier $0.206$) and the EV
head ($+\$4{,}643$ mean), and it is survivable under the honest market-block
bootstrap (P(profit)$=1.000$ over $243$ independent traded markets, $95\%$
CI lower bound $+\$2{,}928$).  The economic edge is therefore real but
...
Splitting the held-out set by predictability, the logistic earns $+\$2{,}398$
in high-predictability and $+\$4{,}619$ in \emph{low}-predictability markets,
\emph{both} at market-block P(profit)$=0.994$---precisely the
low-predictability regime where the world model's settlement head collapses to
$-\$826$ at P(profit)$=0.276$ (Table~\ref{tab:ev_arms}).  A linear readout of
the spot features therefore never exhibits the low-predictability failure at
all: that failure is a property of the settlement head's miscalibration (the
anti-directional mid-probability bins of Fig.~\ref{fig:reliability}), not of
the regime.  This refines---without negating---the EV head
...
% --------------------------------------------------------------------------
\subsection{Direct EV supervision eliminates the low-predictability failure}\label{sec:results_ev}

Broader market coverage (645 BTC markets vs.\ the original 408) exposes a
regime failure latent in the settlement head: P(profit)$=0.276$ and
drawdown $0.257$ in \emph{low-predictability} markets (Table~\ref{tab:ev_arms}).
The mechanism is structural---the settlement head predicts absolute
probability but not whether the \emph{book price already reflects that
information}.  When predictability is low the oracle-lag gap has partially
closed; the book mid is near 0.5 but the head still fires, buying into a
spread it cannot recover.
...
\midrule
Settlement (baseline) & $+3{,}946$ & $+4{,}772$ & $-826$ & $1.000$ & $0.276$ & $5{,}598$ \\
EV Huber only (ours) & $\mathbf{+5{,}565}$ & $+3{,}017$ & $\mathbf{+2{,}548}$ & $1.000$ & $0.998$ & $\mathbf{469}$ \\
EV Huber $+$ action CE & $+3{,}697$ & $+3{,}849$ & $-152$ & $1.000$ & $0.466$ & $4{,}001$ \\
\bottomrule
\end{tabular}
...
regime-robust.  EV Huber only lifts total PnL $+41\%$ and reduces regime
sensitivity $12{\times}$ versus the settlement baseline.}
\label{tab:ev_arms}
\end{table}
...
restoring the low-predictability failure (P(profit)$=0.466$, drawdown
$0.226$).  The minimal configuration---EV Huber only---is therefore the
selected architecture.

\paragraph{Regime-sensitivity.}
The PnL spread between high- and low-predictability markets collapses from
\$5{,}598 under the settlement head to \$469 under direct EV
supervision---a $12{\times}$ reduction.  The training signal is explicitly
...
\midrule
0 & $+5{,}565$ & $+3{,}017$ & $+2{,}548$ & $1.000$ & $0.998$ & $0.091$ \\
1 & $+4{,}318$ & $+2{,}981$ & $+1{,}337$ & $1.000$ & $0.932$ & $0.129$ \\
2 & $+4{,}047$ & $+2{,}234$ & $+1{,}813$ & $1.000$ & $0.982$ & $0.104$ \\
\midrule
Mean & $+4{,}643$ & $+2{,}744$ & $+1{,}899$ & $1.000$ & $0.971$ & $0.108$ \\
\midrule
Settlement & $+3{,}946$ & $+4{,}772$ & $-826$  & $1.000$ & $0.276$ & $0.257$ \\
\bottomrule
\end{tabular}
\caption{Multi-seed confirmation: EV Huber only (lob\_edge\_residual) on 645 held-out
BTC markets, deterministic latent, ER gate $=0.60$.  All three seeds beat the settlement
baseline total PnL ($+\$3{,}946$) and eliminate the catastrophic low-predictability
failure (mean P(profit)$=0.971 \gg 0.276$).  Low-predictability PnL varies
($+\$1{,}337$--$+\$2{,}548$) reflecting seed-dependent EV surface learning, but
remains net positive in all seeds.}
\label{tab:ev_seeds}
\end{table}
...
A 1\textcent/share execution-cost penalty degrades the low-predictability
bootstrap probability from $0.998$ to $0.884$ and raises the 95th-percentile
drawdown from $0.091$ to $0.143$; the strategy remains survivable in both
regimes, unlike the settlement head which fails catastrophically
($P(\text{lo}){=}0.276$, dd95${=}0.257$).
Restricting to the liquid depth band $[60\text{k},130\text{k}]$ removes 36\%
of low-predictability trades and reduces $P(\text{lo})$ to $0.803$, while
high-predictability performance is unchanged ($P{=}1.000$).
Gating out the first 25\% of each market's life (TTE $\geq 0.25$) produces
the mildest degradation: $P(\text{lo}){=}0.980$ with the bootstrap
CI lower bound remaining positive ($+\$109$).
All three stress tests clear the settlement-head threshold by a wide margin.
...
\midrule
Baseline   & ---            & $1.000$ & $0.998$ & $0.091$ \\
Slippage   & 1\textcent/share & $1.000$ & $0.884$ & $0.143$ \\
Depth band & $[60\text{k},130\text{k}]$ & $1.000$ & $0.803$ & $0.131$ \\
Early-market gate & TTE $\geq 0.25$ & $1.000$ & $0.980$ & $0.101$ \\
\bottomrule
\end{tabular}
...
Settlement-head baseline for comparison: $P(\text{lo}){=}0.276$, dd95(lo)${=}0.257$.}
\label{tab:ev_stress}
\end{table}
...
($+\$7{,}016$), so the economic edge is carried by the spot-conditioned
pipeline and selective-participation framing, not by the world-model
architecture.  Second, broader market coverage
(645 markets) exposes a regime failure in the settlement head:
P(profit)$=0.276$ in low-predictability markets.  Supervising the
tradeable expected value directly (BookRelativeEdgeHead, EV Huber only)
converts the $-\$826$ low-predictability loss into $+\$2{,}548$, lifts
total PnL to $+\$5{,}565$ ($+41\%$), and collapses the high-vs-low PnL
spread $12{\times}$ (from $\$5{,}598$ to $\$469$).  The regime axis a
FiLM router will not encode is exactly the one a strategy gate and a
direct-EV head, together, harvest profitably.

\paragraph{Future work.}
The negative motivates the A100-class MIMO headline run the 4080 cannot
host.  The economic positive motivates a live-execution study (the
...
\begin{itemize}[nosep]
  ...
  \item \emph{Mamba-3 MIMO backbone.} Hardware-gated on the RTX~4080; the one
    outstanding A100-class run (Section~\ref{sec:results_ablation}).
\end{itemize}
...
A.~Lahoti, T.~Dao, and A.~Gu.
\newblock Mamba-3: Multi-input multi-output state space models.
...
\bibitem[Nagy et~al.(2023)]{nagy2025lobs5}
...
% Draft entry added for the cross-dataset results; verify exact title/author list against arXiv:2308.01915.
\bibitem[Prata et~al.(2023)]{prata2023lobcast}
M.~Prata et~al.
\newblock {LOBCAST}: A benchmark study of deep-learning models for stock price trend prediction from limit-order books.
