...
class RegimeSupervisionTest(unittest.TestCase):
    ...
    def test_vol_bucket_labels_rank_high_vol_above_low_vol(self):
        ...
        choppy[0, ::2] = 5.0  # Alternating jumps give the tail a large rolling std.
    ...
    def test_supervision_loss_lower_when_logits_match_labels(self):
        ...
        self.assertLess(
            float(regime_supervision_loss(matched, labels)),
            float(regime_supervision_loss(mismatched, labels)),
        )
    def test_conditioning_feature_is_monotonic_in_volatility(self):
        # Within one batch the feature must increase with realized volatility so a linear router head can
        # bucket it into the supervised vol regimes; the choppy tail reads higher than the flat head. The
        # two regions share a batch so the batch standardization has variance to normalize.
        mid = torch.zeros(1, 64)
        mid[0, 32:][::2] = 5.0  # Alternating jumps make the tail high-vol, the head low-vol.
    ...
    def test_conditioning_feature_is_rank_preserving_in_volatility(self):
        # The standardized feature stays an affine, rank-preserving transform of the causal realized vol
        # the supervision label buckets, so feeding it gives the router exactly that axis.
        torch.manual_seed(0)
    ...
    def test_external_vol_lets_supervision_discriminate_better_than_the_latent_proxy(self):
        # Head-to-head: a supervised router that reads the external obs-vol feature must reach a strictly
        # lower per-sample entropy than the same router restricted to its latent-speed proxy, since the
        # external feature is the very axis the supervision label buckets. The proxy router stays near the
        # uniform ceiling (the live failure mode); the fed router commits to regimes.
        def _train(use_external):
            torch.manual_seed(0)
            mod = RegimeFiLMModulator(16, 2, num_regimes=4, embed_dim=8,
                                      condition_on_vol=True, vol_window=4, init_scale=0.03)
        ...
        self.assertLess(fed_entropy, 1.34)  # Below the ln(4)=1.386 uniform ceiling (weakly, in this noisy synthetic; the real latent discriminates far more).
    def test_decouple_router_isolates_router_head_from_film_gradient(self):
        # Decoupling detaches the router logits on the path into the hypernetwork, so a FiLM-only loss
        # leaves the router head untouched (no recon gradient can drag it to uniform) while the
        # hypernetwork that emits FiLM still trains.
        torch.manual_seed(0)
    ...
    def test_coupled_router_passes_film_gradient_into_the_router_head(self):
        # Control: without decoupling the FiLM loss does reach the router head — the very coupling that
        # lets reconstruction pin the router uniform.
        torch.manual_seed(0)
    ...
    def test_per_sample_entropy_separates_balanced_confident_from_degenerate_uniform(self):
        # Each step peaked at a different regime: batch-mean is ~uniform (assignment entropy high) but
        # every step is confident, so per-sample entropy is ~0. This is exactly the distinction regime
        # supervision targets, which the batch-mean diagnostic alone cannot see.
