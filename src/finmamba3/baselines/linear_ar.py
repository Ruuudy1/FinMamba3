...
class LinearARConfig:
    ...
    # Ridge penalty on the lookback x F design matrix: any value >= 1 fixes the ill-conditioning that made the unregularized fit explode on held-out data and collapse the floor to majority-vote.
    ridge: float = 10.0
...
class LinearAR:
    ...
    def _build_design_matrix(self, features: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        ...
        for t in range(L, T - 1):
            ...
            # Regress the next-tick mid RETURN, not the level, so the predicted sign stays meaningful; regressing the level and differencing let any lstsq bias dominate the sign and collapsed the floor to one class.
            targets.append(features[t + 1, mid_idx] - features[t, mid_idx])
        return np.asarray(rows), np.asarray(targets)

    def fit(self, features: np.ndarray) -> None:
        X, y = self._build_design_matrix(features)
        # Ridge-regularizes the high-dimensional flat design matrix (lookback x F columns): an unregularized lstsq is ill-conditioned here and extrapolates to 1e4-scale predictions on held-out data, collapsing the floor to a single class, so augmenting with sqrt(ridge) * I solves the penalized least squares in one stable lstsq call.
    ...
    def predict(self, features: np.ndarray) -> np.ndarray:
        ...
        if features.shape[1] != self.feature_dim:
            raise ValueError(f"Feature dim mismatch: trained on {self.feature_dim}, got {features.shape[1]}")
