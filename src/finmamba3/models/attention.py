...
class MultiHeadAttentionPreNorm(nn.Module):
    """Pre-norm multi-head attention with RMSNorm (modern architecture)."""
    ...
    def forward(self, q, k, v, mask=None):
        ...
        sz_b, len_q, len_k, len_v = q.size(0), q.size(1), k.size(1), v.size(1)
        residual = q
        # Pre-norm: normalize before attention, leaving the residual path unnormalized.
        q = self.layer_norm(q)
        k = self.layer_norm(k) if k is not q else q
        v = self.layer_norm(v) if v is not q else q
        # Project to queries, keys, and values, then split into heads: b x lq x n x dv.
        q = self.w_qs(q).view(sz_b, len_q, n_head, d_k)
        k = self.w_ks(k).view(sz_b, len_k, n_head, d_k)
        v = self.w_vs(v).view(sz_b, len_v, n_head, d_v)
...
class PositionwiseFeedForwardPreNorm(nn.Module):
    """Pre-norm position-wise feed-forward with RMSNorm and SiLU (modern architecture)."""
...
class AttentionBlockPreNorm(nn.Module):
    """Pre-norm self-attention block matching modern transformer architectures."""
...
class AttentionBlockKVCachePreNorm(nn.Module):
    """Pre-norm attention block with KV-cache support for autoregressive rollouts."""
